---
title: "Combined mother and teacher report RI-CLPM for social isolation and ADHD symptoms in childhood"
output:  
  html_document:
    df_print: paged
    toc: yes
...
```{r descriptives for social isolation combined report - age 5}
...
```{r descriptives for ADHD symptoms combined report - age 5}
...
```{r those missing both ADHD and isolation data}
# missing isolation
dat <- dat %>%
...
dat <- dat %>%
  mutate(missing.isolation.mother =
           if_else(
               !is.na(sisoem5) & 
               !is.na(sisoem7) &
               !is.na(sisoem10) &
...
table(dat$missing.isolation.mother)

dat <- dat %>%
  mutate(missing.isolation.teacher =
           if_else(
               !is.na(sisoet5) & 
               !is.na(sisoet7) &
               !is.na(sisoet10) &
...
table(dat$missing.isolation.teacher)
...
table(dat$missing.adhd.mother)

dat <- dat %>%
  mutate(missing.adhd.teacher =
           if_else(
               !is.na(tadhdet5) & 
               !is.na(tadhdet7) &
               !is.na(tadhdet10) &
...
table(dat$missing.adhd.teacher)
...
Autoregressive paths were not constrained to be equal as this showed a significant loss in model fit. Therefore, the constrained model only constrains cross-lag paths (c and d) here (constraint testing of CLPM was left out of this script to make the script as concicse as possible).
...
***
NEED TO ADD IN HERE THE CHI SQUARE BAR TEST. 
***
...
RICLPMcomb2d <- '
  ...
  wad5 ~~ wad5 # Variances
  wsi5 ~~ wsi5 
  wad7 ~~ vad*wad7 # Residual variances
  wsi7 ~~ wsi7 
  wad10 ~~ wad10 
  wsi10 ~~ wsi10 
  wad12 ~~ wad12 
...
### Constrained (co)variances of within-person variables (RICLPMcomb4)

```{r specify RICLPMcomb4}
RICLPMcomb4 <- '
  # Create between components (random intercepts treated as factors here)
  RIad =~ 1*tadhde5 + 1*tadhde7 + 1*tadhde10 + 1*tadhde12 #x
  RIsi =~ 1*sisoe5 + 1*sisoe7 + 1*sisoe10 + 1*sisoe12 #y
  ...
  # Estimate the covariances between the residuals of the within-person centered variables (the innovations) - constrained to be equal
  wad7 ~~ cov*wsi7
  wad10 ~~ cov*wsi10
  wad12 ~~ cov*wsi12 
  ...
  # Estimate the (residual) variance of the within-person centered variables - constrained to be equal
  wad5 ~~ wad5 # Variances
  wsi5 ~~ wsi5 
  wad7 ~~ vad*wad7 # Residual variances - constrain the ad and si variables to have the same variance (not at first time point)
  wsi7 ~~ vsi*wsi7 
  wad10 ~~ vad*wad10 
  wsi10 ~~ vsi*wsi10 
  wad12 ~~ vad*wad12 
...
```{r fit RICLPMcomb4}
RICLPMcomb4.fit <- lavaan(RICLPMcomb4, 
                      data = dat, 
                      missing = 'ML', 
                      meanstructure = TRUE, 
                      int.ov.free = TRUE,
...
RICLPMcomb4.fit.summary <- summary(RICLPMcomb4.fit, 
                                fit.measures = TRUE,
                                standardized = TRUE)

#Table of model fit 
RICLPMcomb4.fit.summary.fit <- table.model.fit(RICLPMcomb4.fit.summary)
RICLPMcomb4.fit.summary.fit
#Table of regression coefficients and covariances (concurrent associations)
RICLPMcomb4.fit.summary.reg <- table.model.coef(model = RICLPMcomb4.fit.summary, type = "RICLPM", constraints = "Yes")
RICLPMcomb4.fit.summary.reg %>% select(lhs, op, rhs, std.all, pvalue)  %>% mutate_if(is.numeric, round, 3)
```

```{r LRT for RICLPMcomb2c and RICLPMcomb4}
lavTestLRT(RICLPMcomb2c.fit, RICLPMcomb4.fit, method = "satorra.bentler.2010") #comparing to other best fitting model
lavTestLRT(RICLPMcomb2d.fit, RICLPMcomb4.fit, method = "satorra.bentler.2010") #comparing to other best fitting model
```

RICLPMcomb4 is *NOT* significantly worse fit (just) compared to RICLPMcomb2c (p = 0.06916) or RICLPM2d (p = 0.05154). The RICLPMcomb4 parameter estimates did not change very much compared to  RICLPMcomb2d. 

## Comparison of CLPM and RI-CLPM using the chi squre bar test

When constraints are placed on the bound of the parameter space, we should use the chi-bar-square test (χ¯2-test) (Stoel et al. 2006 to compare CLPM and RI-CLPM.  

```{r RI-CLPM with RI variances and covariances constrained to zero - equivalent to CLPM}
CLPMcombRI0 <- '
  # Create between components (random intercepts treated as factors here)
  RIad =~ 1*tadhde5 + 1*tadhde7 + 1*tadhde10 + 1*tadhde12 #x
  RIsi =~ 1*sisoe5 + 1*sisoe7 + 1*sisoe10 + 1*sisoe12 #y
  ...
  wad5 ~~ wad5 # Variances
  wsi5 ~~ wsi5 
  wad7 ~~ wad7 # Residual variances - constrain the ad and si variables to have the same variance (not at first time point)
  wsi7 ~~ wsi7 
  wad10 ~~ wad10 
  wsi10 ~~ wsi10 
  wad12 ~~ wad12 
...
CLPMcombRI0.fit <- lavaan(CLPMcombRI0, 
                      data = dat, 
                      missing = 'ML', 
                      meanstructure = TRUE, 
                      int.ov.free = TRUE,
...
According to the Chi bar square test, this is very significant, so this means the RI-CLPM fits the data significantly worse than the CLPM? But all the fit statistics say otherwise? 

```{r chi square bar test comb}
library(ChiBarSq.DiffTest)

## Check which indices you need to get the covariance matrix of the random intercepts. 
vcov(RICLPMcomb.fit) # Full covariance matrix

## The 22nd and the 23rd indices regard the random intercepts
indices <- c(17, 18) 
q <- length(indices) # Number of random intercepts
S <- vcov(RICLPMcomb.fit)[indices, indices] # Extract full covariance matrix of the random intercepts

# Step 3
Chi2_clpm <- summary(CLPMcombRI0.fit, fit.measures = TRUE)[1]$FIT[c("chisq")] # Extract chi-square value of CLPM
Chi2_riclpm <- summary(RICLPMcomb.fit, fit.measures = TRUE)[1]$FIT[c("chisq")] # Extract chi-square value of RI-CLPM

df_clpm <- summary(CLPMcombRI0.fit, fit.measures = TRUE)[1]$FIT[c("df")] # Extract df of CLPM
df_riclpm <- summary(RICLPMcomb.fit, fit.measures = TRUE)[1]$FIT[c("df")] # Extract df of RI-CLPM
...
summary(ChiBar2DiffTest, digits = 10)
round(ChiBar2DiffTest$p_value, 4)
```
...
```{r LRT for CLPMcomb_hyp and CLPMcomb_hyp2}
...
```{r LRT for CLPMcomb_inat and CLPMcomb_inat2}
