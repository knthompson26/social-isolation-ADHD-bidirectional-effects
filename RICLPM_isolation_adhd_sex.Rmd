...
table.model.coef <- function(model, sex, constraints){
  if (sex == "Male" & constraints == "No"){
    model.coef <- as.tibble(model$PE[c(86:101),]) %>% select(-block, -group, -exo, -std.lv, -std.nox)
    return(model.coef)
  } else if(sex == "Female" & constraints == "No"){
    model.coef <- as.tibble(model$PE[c(17:32),]) %>% select(-block, -group, -exo, -std.lv, -std.nox)
    return(model.coef)
  } else if(sex == "Male" & constraints == "Yes"){
    model.coef <- as.tibble(model$PE[c(86:101),]) %>% select(-block, -group, -exo, -label, -std.lv, -std.nox)
    return(model.coef)
  } else if(sex == "Female" & constraints == "Yes"){
    model.coef <- as.tibble(model$PE[c(17:32),]) %>% select(-block, -group, -exo, -label, -std.lv, -std.nox)
    return(model.coef)
...
lavTestLRT(RICLPM_inat.lag.c.fit, RICLPM_inat.sex.lag.c.fit)
```

The chi-square difference test of these two nested models is *non significant* (p=0.254). 
...
The chi-square difference test of these two nested models is *NON significant* (p=0.543). **Therefore, we conclude there are no sex differences in the constrained RICLPM teacher report, total ADHD symptoms.** 
