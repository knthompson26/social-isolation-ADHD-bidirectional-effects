...
```{r results table for prosocial and total adhd}
# Model fit 
...
```{r results table for random intercept prosocial and total adhd}
# Model fit 
...
```{r results table for prosocial and hyperactivity adhd}
# Model fit 
...
```{r results table for random intercept prosocial and hyperactivity adhd}
# Model fit 
...
```{r results table for combined report prosocial and total adhd}
# Model fit 
...
```{r results table for random intercept prosocial and inattention adhd}
# Model fit 
...
med.long2.asb.fit.summary.reg %>%
  select(lhs, op, rhs, label, std.all, pvalue)
```
Regressions to double check overall association between antisocial behaviour, social isolation, and ADHD symptoms. 

```{r regressions to double check the overall associations with asb, si, and adhd}
# 5 to 7
reg.si.adhd.asb57 <- lm(sisoe7 ~ tadhde5 + asbe5, data = dat)
summary(reg.si.adhd.asb57)

# 7 to 10
reg.si.adhd.asb710 <- lm(sisoe10 ~ tadhde7 + asbe7, data = dat)
summary(reg.si.adhd.asb710)

# 10 to 12
reg.si.adhd.asb1012 <- lm(sisoe12 ~ tadhde10 + asbe10, data = dat)
summary(reg.si.adhd.asb710)
```
...
```{r results table for antisocial and hyperactivity adhd}
# Model fit 
...
```{r results table for random intercept antisocial and hyperactivity adhd}
# Model fit 
...
```{r results table for antisocial and inattention adhd}
# Model fit 
...
```{r results table for random intercept antisocial and inattention adhd}
# Model fit 
ri.med_inat.long.asb.full2.fit.summary.fit <- table.model.fit(ri.med_inat.long.asb.full2.fit.summary)
# Coefficients -
ri.med_inat.long.asb.full2.fit.summary.reg <- table.model.coef(model = ri.med_inat.long.asb.full2.fit.summary, step = "S3") %>% mutate_if(is.numeric, round, 3)

ri.med_inat.long.asb.full2.fit.summary.reg %>%
  select(lhs, op, rhs, std.all, pvalue) %>%
  filter(lhs == "indirect1" | lhs == "indirect2")
