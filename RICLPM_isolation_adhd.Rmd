...
#Table of model fit 
CLPM.fit.summary.fit <- as.data.frame(t(as.data.frame(CLPM.fit.summary$FIT)))[,c(3,4,9,10,13,14,16,17,18,19,21)]

#Table of regression coefficients and covariances (concurrent associations)
CLPM.fit.summary.reg <- as.tibble(CLPM.fit.summary$PE[c(1:16),-c(4)])
```
...
RICLPM.fit.summary.reg <- as.tibble(RICLPM.fit.summary$PE[c(17:32),-c(4)])
```
...
RICLPM2 is significantly *worse* fit compared to RICLPM according to the Chi square test. However the Chi square is highly dependent on sample size. Also goo to look at difference in model fit. 
 - RMSEA difference = increase smaller than 0.01 is acceptable 
 - CFA difference = decrease smaller than 0.01 is acceptable 
 - TLI difference = decrease smaller than 0.01 is acceptable

Here the CFI is a bigger decrease than we would like - therefore we will continue with the other constraints on the model. 

```{r compare fit RICLPM and RICLPM2}
summary(semTools::compareFit(RICLPM.fit, RICLPM2.fit, nested = TRUE)) #† indicates the best fitting model - using the robust statistics
```
...
# Table of regression coefficients and covariances 
RICLPM2d.fit.summary.reg <- as.tibble(RICLPM2d.fit.summary$PE[c(17:32),-c(4,5)])
```
...
#Table of regression coefficients and covariances
RICLPM_hyp2d.fit.summary.reg <- as.tibble(RICLPM_hyp2d.fit.summary$PE[c(17:32),-c(4,5)])
```
...
#Table of regression coefficients and covariances
RICLPM_inat.fit.summary.reg <- as.tibble(RICLPM_inat.fit.summary$PE[c(17:32),-c(4)])
```
...
#Table of regression coefficients and covariances
RICLPM_inat2d.fit.summary.reg <- as.tibble(RICLPM_inat2d.fit.summary$PE[c(17:32),-c(4,5)])
```
...
#Table of model fit 
CLPMt.fit.summary.fit <- as.data.frame(t(as.data.frame(CLPMt.fit.summary$FIT)))[,c(3,4,9,10,13,14,16,17,18,19,21)]

#Table of regression coefficients and variances
CLPMt.fit.summary.reg <- as.tibble(CLPMt.fit.summary$PE[c(1:16),-c(4)])
```
...
#Table of regression coefficients and variances
RICLPMt.fit.summary.reg <- as.tibble(RICLPMt.fit.summary$PE[c(17:32),-c(4)])
```
...
#Table of regression coefficient 
RICLPMt2.fit.summary.reg <- as.tibble(RICLPMt2.fit.summary$PE[c(17:32),-c(4,5)])
```
...
#Table of regression coefficients and covariances
RICLPMt_hyp.fit.summary.reg <- as.tibble(RICLPMt_hyp.fit.summary$PE[c(17:32),-c(4)])
```
...
#Table of model fit 
RICLPMt_hyp2.fit.summary.fit <- as.data.frame(t(as.data.frame(RICLPMt_hyp2.fit.summary$FIT)))[,c(3,4,9,10,13,14,16,17,18,19,21)]

#Table of regression coefficients and covariances 
RICLPMt_hyp2.fit.summary.reg <- as.tibble(RICLPMt_hyp2.fit.summary$PE[c(17:32),-c(4,5)])
```
...
However, the model fit actually shows that the RMSEA and TLI are better fit, and the CFI only had a decrease of 0.01 which is considered acceptable. 

```{r compare fit RICLPMt_hyp and RICLPMt_hyp2}
summary(semTools::compareFit(RICLPMt_hyp.fit, RICLPMt_hyp2.fit, nested = TRUE)) #† indicates the best fitting model - using the robust statistics
```
...
#Table of regression coefficient 
RICLPMt_inat.fit.summary.reg <- as.tibble(RICLPMt_inat.fit.summary$PE[c(17:32),-c(4)])
```
...
#Table of model fit 
RICLPMt_inat2.fit.summary.fit <- as.data.frame(t(as.data.frame(RICLPMt_inat2.fit.summary$FIT)))[,c(3,4,9,10,13,14,16,17,18,19,21)]

#Table of regression coefficient 
RICLPMt_inat2.fit.summary.reg <- as.tibble(RICLPMt_inat2.fit.summary$PE[c(17:32),-c(4,5)])
```
...
```{r compare fit RICLPMt_hyp and RICLPMt_hyp2}
summary(semTools::compareFit(RICLPMt_inat.fit, RICLPMt_inat2.fit, nested = TRUE)) #† indicates the best fitting model - using the robust statistics
```
