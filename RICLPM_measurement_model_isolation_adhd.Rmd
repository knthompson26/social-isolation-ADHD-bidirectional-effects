...
RICLPM_multi_inat_S1 <- '
  ...
  RIinat1 + RIinat2 + RIinat3 + RIinat4 + RIinat5 + RIinat6 + RIinat7 + RIinat8 + RIsi1 + RIsi2 + RIsi3 + RIsi4 + RIsi5 + RIsi6 ~~ 0*WFsi5 + 0*WFinat5
'
```
...
**Significantly worse fit to include the restrictions, p=0.00103)**
...
- PE13M 5&10 Not liked by other children (social isolation; n)
- PE4M  5&12 Doesnt get along with other children (social isolation; k)
- PE11M 5&10 Would rather be alone than with others (social isolation; m)
- PE90M 5&10 Loses things, like toys and clothes (inattention; h)
...
```
*The MIi model is still significantly worse fit compared to the configural model, p=0.005928*
...
```
*The MIl (and i) freed model is still significantly worse fit compared to the configural model, p=0.02484*
...
```
**This model does not fit significantly worse compared to the configural invaraince model (p=0.09866)**
...
RICLPM_multi_hyp_S1 <- '
  ...
  # Constrain covariance of the between factors and exogenous within factors to 0
  RIhyp1 + RIhyp2 + RIhyp3 + RIhyp4 + RIhyp5 + RIhyp6 + RIhyp7 + RIhyp8 + RIsi1 + RIsi2 + RIsi3 + RIsi4 + RIsi5 + RIsi6 ~~ 0*WFsi5 + 0*WFhyp5
'
```
